<script lang="ts">
  ...
  type SymDef = {
    ...
    boardCount: number
    label: string
    pay: number        // base multiplier
    weight: number     // makin kecil makin langka
  }

  const SYMDEFS: SymDef[] = [
    { id:'apple',  emoji:'🍎', boardCount:3, label:'Apel',     pay:5,   weight:22 },
    { id:'orange', emoji:'🍊', boardCount:3, label:'Jeruk',    pay:10,  weight:18 },
    { id:'wmelon', emoji:'🍉', boardCount:3, label:'Semangka', pay:15,  weight:14 },
    { id:'melon',  emoji:'🍈', boardCount:2, label:'Melon',    pay:20,  weight:12 },
    { id:'frog',   emoji:'🐸', boardCount:3, label:'Kodok',    pay:20,  weight:10 },
    { id:'star',   emoji:'⭐', boardCount:3, label:'Bintang',  pay:30,  weight:8  },
    { id:'sev77',  emoji:'77', boardCount:2, label:'77',       pay:40,  weight:6  },
  ...
  type VolTier = {
    ...
    label: string
    desc: string
    hitRate: number    // peluang menang per spin
    maxWinMult: number // cap maksimal win dalam kelipatan bet
    rtp: number        // 0.94 = 94% RTP
  }
  ...
  const VOL_TIERS: VolTier[] = [
    {
      id: 'low',
      label: 'Low',
      desc: 'Sering menang, hadiah kecil',
      hitRate: 0.35,
      maxWinMult: 50,
      rtp: 0.94,
    },
    ...
    },
    {
      id: 'high',
      label: 'High',
      desc: 'Jarang menang, hadiah BESAR',
      hitRate: 0.10,
      maxWinMult: 1000,
      rtp: 0.94,
    },
  ...
  // ── Near miss config ───────────────────────────────
  // Kalau kalah, 30% chance landed 1 posisi sebelum simbol picked
  // → member merasa "hampir!"
  ...
  function makeBoard(): (string|null)[][] {
    ...
    for (const def of SYMDEFS) {
      for (let i=0; i<def.boardCount; i++) pool.push(def.id)
    }
  ...
  // ── RTP-based win calculation ──────────────────────
  // Pure RNG — tidak ada schedule
  // Volatility tier yang kontrol hit rate & max win
  // RTP dijaga konsisten di 94%

  function calcWin(betAmount: number, symId: string, cnt: number, vol: VolTier): number {
    const def = BY_ID[symId]
    // base payout dari simbol
    const base = betAmount * def.pay
    // bonus multiplier kalau simbol banyak di board
    const countMult = cnt >= 6 ? 8 : cnt >= 5 ? 4 : cnt >= 4 ? 2 : 1
    const raw = base * countMult
    // cap oleh volatility maxWinMult
    const capped = Math.min(raw, betAmount * vol.maxWinMult)
    // potong agar sesuai RTP (94% dari expected value)
    // expected value per spin = hitRate × avgPayout
    // avgPayout harus = bet × rtp / hitRate
    const targetAvgPayout = betAmount * vol.rtp / vol.hitRate
    // kalau raw > target, potong proporsional
    const adjusted = Math.min(capped, targetAvgPayout * 2.5)
    return Math.floor(adjusted)
  }

  // ── Near miss logic ────────────────────────────────
  // Saat kalah, kadang berhenti 1 posisi sebelum simbol picked
  function getLosingSteps(currentPos: number, board: (string|null)[][], picked: string): number {
    const total = BORDER_POS.length
    // near miss: 30% chance berhenti 1 langkah sebelum picked
    if (Math.random() < NEAR_MISS_CHANCE) {
      const winPositions = BORDER_POS
        .map((p,i) => ({i, sym: board[p.r][p.c]}))
        .filter(x => x.sym === picked)
      if (winPositions.length > 0) {
        const target = winPositions[Math.floor(Math.random()*winPositions.length)]
        // berhenti 1 posisi SEBELUM simbol picked (near miss!)
    ...
    // lose biasa: berhenti di simbol random yang bukan picked
    const losePositions = BORDER_POS
      .map((p,i) => ({i, sym: board[p.r][p.c]}))
      .filter(x => x.sym !== picked)
  ...
  // Shuffle board manual — simbol fixed per sesi, bisa di-shuffle kalau mau
  ...
  function playLose() {
    ...
    setTimeout(()=>playTone(180,0.15,'sawtooth',0.15),100)
  }
  ...
  function selectPick(id: string) {
    if (spinning) return
    picked = id; playClick()
  }
  function selectVol(id: string) {
    if (spinning) return
    volId = id; playClick()
  }
  ...
  function startAuto() {
    ...
    autoSpin = true; autoLeft = autoCount; runAuto()
  }
  function stopAuto() {
  ...
  async function runAuto() {
    ...
    autoLeft--
    await doSpin()
    if (autoSpin && autoLeft > 0 && credit >= bet) {
      autoTimer = setTimeout(runAuto, autoDelay)
    } else {
      stopAuto()
    }
  }
  ...
  async function doSpin() {
    ...
    const vol = todayVol
    // Pure RNG — hanya hitRate yang tentukan menang/kalah
    ...
    await new Promise<void>(resolve => {
      let step = 0
      function tick() {
        currentPos = (currentPos+1) % BORDER_POS.length
        step++
        ...
        if (step < steps) setTimeout(tick, speed)
        else resolve()
      }
      setTimeout(tick, 55)
    })
    ...
    if (landed === picked) {
      ...
      credit += win; winAmount = win
      centerNum = String(win); centerLabel = 'WIN!'
      flashWin = true; playWin(win)
      totalWin++; totalBet += bet; totalPayout += win
      history = [{
        round, result:'WIN', sym:picked, emoji:def.emoji,
        bet, win, credit, vol: vol.label
      }, ...history].slice(0,50)
      ...
      // CEK near miss — landed 1 posisi sebelum picked
      const landedPos = currentPos
      const nextPos = (landedPos+1) % BORDER_POS.length
      const nextSym = board[BORDER_POS[nextPos].r][BORDER_POS[nextPos].c]
      const prevPos = (landedPos-1+BORDER_POS.length) % BORDER_POS.length
      const prevSym = board[BORDER_POS[prevPos].r][BORDER_POS[prevPos].c]

      if (nextSym === picked || prevSym === picked) {
        // NEAR MISS
        isNearMiss = true
        centerNum = '😱'; centerLabel = 'Hampir!'
        msg = `Hampir! ${def.emoji} ada di sebelahnya...`
        playNearMiss()
        totalLose++; totalNear++; totalBet += bet
      } else {
        // KALAH biasa
        centerNum = String(cnt)
        centerLabel = `${def.label} ada ${cnt} kotak`
        msg = 'Tidak menang. Coba lagi!'
        playLose()
        totalLose++; totalBet += bet
      }

      history = [{
        round,
        result: isNearMiss ? 'NEAR' : 'LOSE',
        sym:picked, emoji:def.emoji,
        bet, win:0, credit, vol: vol.label
      }, ...history].slice(0,50)
    }

    spinning = false
    if (credit <= 0) {
      setTimeout(()=>{ credit=100000; msg='Credit diisi ulang!'; playCoin() },2000)
    }
  }
</script>
...
<div class="game">
  ...
  <div class="top-bar">
    <div>
      ...
      <div class="led">{bet.toLocaleString()}</div>
    </div>
    <div class="title-box">
      <div class="title">BOARD JACKPOT</div>
      <div class="subtitle">Pragmatic Style</div>
      <button class="btn-shuffle" onclick={shuffleBoard} disabled={spinning} title="Acak posisi simbol">
        🔀 Shuffle Board
      </button>
    </div>
    <div style="text-align:right">
      <div class="lbl">CREDIT</div>
      <div class="led">{credit.toLocaleString()}</div>
    </div>
  </div>
  ...
  <div class="vol-wrap">
    {#each VOL_TIERS as v}
      ...
      >
        ...
        <div class="vol-stats">
          Hit {Math.round(v.hitRate*100)}% · Max {v.maxWinMult}×
        </div>
      </button>
    {/each}
  </div>
        ...
        {#each Array.from({length:COLS},(_,c)=>c) as c}
              ...
              <div class="center-sym-wrap">
                {#if def.id === 'bar'}
                  <div class="center-bar">
                    <div class="cb-top">BAR</div>
                    <div class="cb-mid">×100</div>
                    <div class="cb-bot">BAR</div>
                  </div>
          ...
          {:else if isBorderCell(r,c)}
            ...
            <div
              ...
              class:near-cell={isNearMiss && (
                posIdx === (currentPos+1) % BORDER_POS.length ||
                posIdx === (currentPos-1+BORDER_POS.length) % BORDER_POS.length
              ) && symId === picked}
            >
              {#if def}
                {#if def.id === 'bar'}
  ...
  <div class="paytable">
    {#each SYMDEFS as def}
      <button
        class="pay-btn"
        class:selected={def.id === picked}
        onclick={() => selectPick(def.id)}
        disabled={spinning}
      >
        {#if def.id === 'bar'}
          <div class="pt-bar">
            <div class="ptb-top">BAR</div>
            <div class="ptb-mid">×100</div>
          </div>
        {:else if def.id === 'sev77'}
      ...
      <div class="auto-group">
        ...
        <div class="auto-opts">
          {#each AUTO_OPTIONS as n}
            <button class="auto-opt" class:selected={autoCount===n}
              onclick={()=>autoCount=n} disabled={autoSpin}>{n}×</button>
          {/each}
        </div>
      </div>
      <div class="auto-group">
        ...
        <div class="auto-opts">
          {#each DELAY_OPTIONS as d}
            <button class="auto-opt" class:selected={autoDelay===d.ms}
              onclick={()=>autoDelay=d.ms} disabled={autoSpin}>{d.label}</button>
          {/each}
        </div>
      </div>
    </div>
  ...
  <div class="controls">
    <div class="bet-ctrl">
      ...
      <div style="display:flex;gap:4px;align-items:center;margin-top:2px">
        ...
        <div class="bet-val">{bet.toLocaleString()}</div>
        <button class="btn" onclick={()=>changeBet(1)} disabled={spinning}>+</button>
      </div>
    </div>
    ...
    <div style="text-align:center">
      ...
      <div class="win-val">{winAmount.toLocaleString()}</div>
    </div>
  </div>
  ...
  <div class="stats-bar">
    <div class="stat-box win-stat">
      ...
      <div class="stat-val">{totalWin}</div>
    </div>
    <div class="stat-box near-stat">
      <div class="stat-label">NEAR MISS</div>
      <div class="stat-val">{totalNear}</div>
    </div>
    <div class="stat-box lose-stat">
      <div class="stat-label">LOSE</div>
      <div class="stat-val">{totalLose}</div>
    </div>
    <div class="stat-box">
  ...
  <div class="profit-wrap">
    <div class="profit-box house-box">
      ...
      <div class="profit-sub">dari {totalBet.toLocaleString()} total bet · RTP {totalBet>0?Math.round(totalPayout/totalBet*100):0}%</div>
    </div>
    <div class="profit-box member-box">
      ...
      <div class="profit-val" class:profit-pos={memberProfit>=0} class:profit-neg={memberProfit<0}>
        {memberProfit >= 0 ? '+' : ''}{memberProfit.toLocaleString()}
      </div>
      <div class="profit-sub">payout {totalPayout.toLocaleString()} · {round} spin</div>
    </div>
  </div>
  {/if}
    ...
    <div class="history-list">
      {#each history as h}
        <div class="h-row" class:h-win={h.result==='WIN'} class:h-lose={h.result==='LOSE'} class:h-near={h.result==='NEAR'}>
          ...
          <div class="h-badge"
            class:badge-win={h.result==='WIN'}
            class:badge-lose={h.result==='LOSE'}
            class:badge-near={h.result==='NEAR'}>
          ...
          <div class="h-detail">
            Bet <span class="h-num">{h.bet.toLocaleString()}</span>
            {#if h.result==='WIN'}→ <span class="h-win-num">+{h.win.toLocaleString()}</span>{/if}
          </div>
          <div class="h-credit">{h.credit.toLocaleString()}</div>
        </div>
      {/each}
    </div>
  </div>
...
<style>
  *{box-sizing:border-box}
  ...
  .led{background:#1a0000;color:#ff3333;font-size:24px;padding:8px 16px;border-radius:6px;min-width:100px;text-align:center;letter-spacing:2px;font-family:monospace}
  ...
  .cell.near-cell{background:#3a0a00;border-color:#ff4444;box-shadow:0 0 12px #ff4444,0 0 24px #ff444466;animation:nearpulse 0.4s ease-in-out 3}
  ...
  .profit-val{font-size:28px;font-weight:700;font-family:monospace;line-height:1}
